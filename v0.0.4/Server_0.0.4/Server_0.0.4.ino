...
char * objectToJson( JsonObject& o ){
  char s[bufferSize];
  memset(s, '\0', bufferSize);   // this is necessary to set the char* with an ending zero, otherwise there will be a stack overflow
...
void cycle(){
  if( relayId > -1 && local_time % 1 == 0 ){
    JsonObject& data = JSONBuffer.createObject();
    data["type"] = 2;   // switch for simple relay
    data["text"] = relayStatus;
    server.sendTXT(0, objectToJson( data ));
...
void doSomething(uint8_t num, WStype_t type, uint8_t * payload, size_t lenght) {
    ...
    switch(type) {
        ...
        case WStype_CONNECTED:
          {
            JsonObject& data = JSONBuffer.createObject();
            data["type"] = 0;     // Force to sign in
            data["text"] = num;   // the id of the new module
